...
HT_tuning_base <- function( pinputexps, bypass=FALSE)
{
  ...
  param_local$lgb_param <- list(
    ...
    objective = "binary", # ESTE NO
    metric = "custom", # ESTE NO
    first_metric_only = TRUE, # ESTE NO
    boost_from_average = TRUE, # ESTE NO
    feature_pre_filter = FALSE, # ESTE NO
    ...
    min_sum_hessian_in_leaf = c(0,20), #  min_sum_hessian_in_leaf >= 0.0
    ...
    is_unbalance = c(FALSE, TRUE), # Puede ser FALSE o TRUE
  ...
  param_local$bo_iteraciones <- 200 # iteraciones de la Optimizacion Bayesiana -> Originalmente estaba en 60
